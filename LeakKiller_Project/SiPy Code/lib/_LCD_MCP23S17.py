...
class MCP23S17Backend:
    ...
    def Read(self, Reg): # this shoul be trie out on its own.
        self.SPI_CS(True)
        self.SPI_CS(False) # Active Low
        read_buf = bytearray(3)
        ReadAddress = (self.ChipAddress + 0x01) # the read address has a read bit that needs to be enabled
        bytesread = self.SPI_Comm.write_readinto(bytes([ReadAddress, Reg, 0xFF]), read_buf) # Redundant data is needed to clock out the register
        self.SPI_CS(True)
        return read_buf[2]
